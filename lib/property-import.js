...
// Criterio do Jales (STR): "<=20 min de Disney/Universal" — pontos de referencia
// usados para estimar tempo de carro real via OSRM (sem custo, sem API key)
const THEME_PARKS = [
  { name: "Walt Disney World", lat: 28.3852, lng: -81.5639 },
  { name: "Universal Orlando", lat: 28.4743, lng: -81.4677 }
];
...
async function driveMinutes(lat, lng, destLat, destLng) {
  const url = `${OSRM_ROUTE_URL}/${lng},${lat};${destLng},${destLat}?overview=false`;
...
async function lookupClosestParkMinutes(lat, lng) {
  let best = null;
  for (const park of THEME_PARKS) {
    const minutes = await driveMinutes(lat, lng, park.lat, park.lng);
    if (minutes != null && (best == null || minutes < best.minutes)) {
      best = { minutes, park: park.name };
    }
  }
  return best;
}
...
async function enrichProperty(p) {
  ...
  if (p.driveMinutesToParks == null && p.lat != null && p.lng != null) {
    try {
      const closest = await lookupClosestParkMinutes(p.lat, p.lng);
      if (closest) {
        p.driveMinutesToParks = Math.round(closest.minutes);
        p.closestPark = closest.park;
        process.stderr.write(`  drive time (OSRM): ${p.address} -> ${Math.round(closest.minutes)}min de ${closest.park}\n`);
      } else {
        process.stderr.write(`  AVISO: OSRM nao retornou rota para "${p.address}"\n`);
      }
    } catch (e) {
      process.stderr.write(`  AVISO: erro ao consultar OSRM para "${p.address}": ${e.message}\n`);
    }
  }
